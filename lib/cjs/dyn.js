    ...
    updateItem(updateInput, skipPreThrottle = false) {
        ...
        return limit.removeTokens(estimatedConsumption).then(() => new Promise((resolve, reject) => {
            this.dyn.updateItem(Object.assign({
                ...
                if (err) {
                    (0, logger_1.log)('updateItem failed', updateInput.TableName);
                    reject(err);
                    return;
                }
                resolve(data);
            ...
            if (usedCapacity > estimatedConsumption) {
                (0, logger_1.log)('our consumption estimate was wrong for updateItem.', { estimatedConsumption, usedCapacity, table: updateInput.TableName });
    ...
    putItem(putItemInput, skipPreThrottle = false) {
        ...
        return limit.removeTokens(estimatedConsumption).then(() => new Promise((resolve, reject) => {
            this.dyn.putItem(Object.assign({
                ...
                if (err) {
                    (0, logger_1.log)('putItem failed', putItemInput.TableName);
                    reject(err);
                    return;
                }
                resolve(data);
            ...
            const usedCapacity = data.ConsumedCapacity.CapacityUnits;
            if (usedCapacity > estimatedConsumption) {
                (0, logger_1.log)('our consumption estimate was wrong for putItem.', { estimatedConsumption, usedCapacity, table: putItemInput.TableName });
                await limit.removeTokens(usedCapacity - estimatedConsumption);
            }
            if (!data.Attributes) {
                return null;
    ...
    getItem(getInput, skipPreThrottle = false) {
        ...
        return limit.removeTokens(estimatedConsumption).then(() => new Promise((resolve, reject) => {
            this.dyn.getItem(Object.assign({
                ...
                if (err) {
                    (0, logger_1.log)('getItem failed', getInput.TableName);
                    reject(err);
                    return;
                }
                resolve(data);
            ...
            const usedCapacity = data.ConsumedCapacity.CapacityUnits;
            if (usedCapacity > estimatedConsumption) {
                (0, logger_1.log)('our consumption estimate was wrong for getItem.', { estimatedConsumption, usedCapacity, table: getInput.TableName });
                await limit.removeTokens(usedCapacity - estimatedConsumption);
            }
            if (!data.Item) {
                return null;
    ...
    queryTable(queryInput, next, skipPreThrottle = false) {
        ...
        return limit.removeTokens(estimatedConsumption).then(() => new Promise((resolve, reject) => {
            this.dyn.query(Object.assign({
                ...
                if (err) {
                    (0, logger_1.log)('queryTable failed', queryInput.TableName);
                    reject(err);
                    return;
                }
                resolve(data);
            ...
            const usedCapacity = data.ConsumedCapacity.CapacityUnits;
            if (usedCapacity > estimatedConsumption) {
                (0, logger_1.log)('our consumption estimate was wrong for queryTable.', { estimatedConsumption, usedCapacity, table: queryInput.TableName });
                await limit.removeTokens(usedCapacity - estimatedConsumption);
            }
    ...
    scanTable(queryInput, next) {
        ...
        return limit.removeTokens(estimatedConsumption).then(() => new Promise((resolve, reject) => {
            this.dyn.scan(Object.assign({
                ...
                if (err) {
                    (0, logger_1.log)('scanTable failed', err);
                    reject(err);
                    return;
                }
                resolve(data);
    ...
    async batchGet(batchInput, skipPreThrottle = false) {
        ...
        return limit.removeTokens(estimatedConsumption).then(() => new Promise((resolve, reject) => {
            this.dyn.batchGetItem(Object.assign({
                ...
                if (err) {
                    (0, logger_1.log)('batchGet failed', err);
                    reject(err);
                    return;
                }
                resolve(data);
            ...
            if (usedCapacity > estimatedConsumption) {
                (0, logger_1.log)('our consumption estimate was wrong for batchGet.', { estimatedConsumption, usedCapacity, table: tableNames[0] });
                await limit.removeTokens(usedCapacity - estimatedConsumption);
            }
            let out = [];
    ...
    async batchWrite(batchInput, skipPreThrottle = false) {
        ...
        return limit.removeTokens(estimatedConsumption).then(() => new Promise((resolve, reject) => {
            this.dyn.batchWriteItem(Object.assign({
                ...
                if (err) {
                    (0, logger_1.log)('batchWrite failed', err);
                    reject(err);
                    return;
                }
                resolve(data);
